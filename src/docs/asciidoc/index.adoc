...
예를 들어, 잘못된 요청으로 TIL를 만들려고 했을 때 다음과 같은 `400 Bad Request` 응답을 받는다.
...
`GET` 요청을 사용하여 서비스의 모든 TIL를 조회할 수 있다.

operation::query-tils[snippets='http-request,request-headers,request-fields,http-response,response-headers,response-fields,links']

[[resources-tils-create]]
=== TIL 생성

`POST` 요청을 사용해서 새 TIL를 만들 수 있다.
...
`Get` 요청을 사용해서 기존 TIL 하나를 조회할 수 있다.

operation::get-an-til[snippets='http-request,request-headers,request-fields,http-response,response-headers,response-fields,links']

[[resources-tils-update]]
=== TIL 수정

`PUT` 요청을 사용해서 기존 TIL를 수정할 수 있다.

operation::update-til[snippets='http-request,request-headers,request-fields,http-response,response-headers,response-fields,links']
